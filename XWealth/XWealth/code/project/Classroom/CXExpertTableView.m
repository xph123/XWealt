...
@implementation CXExpertTableView
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self)
    {
        ...
        [promptButton setTitle: @"暂无资料！" forState: UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CXExpertCell *cell = [tableView dequeueReusableCellWithIdentifier:InformationCellIdentifier];
    if (cell == nil) {
        cell = [[CXExpertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:InformationCellIdentifier];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_sourceDatas.count!=0) {
        CXExpertModel *model = [_sourceDatas objectAtIndex:indexPath.row];
        CXExpertCellFrame *cellFrame = [[CXExpertCellFrame alloc] initWithDataModel:model];
