//
//  CXSearchProductTransferViewController.m
//  XWealth
//
...
@interface CXSearchProductTransferViewController ()
{
...
@implementation CXSearchProductTransferViewController
...
- (void)viewDidLoad
{
    ...
    CGRect frameView = CGRectMake(kDefaultMargin, kHeightOfSearchBar + kDefaultMargin, self.view.bounds.size.width - 2 * kDefaultMargin, 180);
    
    CXSearchProductTransferView *conditionView = [[CXSearchProductTransferView alloc] initWithFrame:frameView];
...
- (void)didSelectItemAtIndex:(id)data
{
    ...
    CXProductModel *model = [_productList objectAtIndex:indexPath.row];
    self.productModel=model;
    __unsafe_unretained CXSearchProductTransferViewController *weak_self = self;
    ...
    {
        if (model.category==4||model.category==3) {
            if (modelBlk) {
                modelBlk(_productModel);
            }
            [self.navigationController popViewControllerAnimated:YES];
