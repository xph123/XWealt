...
-(void)layoutSubviews {
    ...
    titleFrame.origin.y -= self.frame.size.height/5;
    titleLabel.frame = titleFrame;
    
    CGRect failFrame = labelFrame;
    failFrame.origin.y += self.frame.size.height/5;
...
-(void)setup {
    ...
    if ([self.backImage isEqualToString:@""]||self.backImage==nil) {
        bg = [UIImage imageNamed:@"circle_outline"];
    }
    else
    {
        bg = [UIImage imageNamed:self.backImage];
    }
    ...
    percentLayer.color = UIColorFromRGB(0xFFB312);
    if (self.lineSize!=0) {
        percentLayer.innerRadius = bg.size.width/2-self.lineSize;
    }
    else
    {
        percentLayer.innerRadius = bg.size.width/2-4;
    }
    percentLayer.outerRadius = bg.size.width/2;
    percentLayer.frame = CGRectMake(0, 0, bg.size.width, bg.size.height);
...
//    [titleLabel setTextColor:[UIColor colorWithRed:89/255.0 green:89/255.0 blue:89/255.0 alpha:1.0]];
    ...
    [percentLabel setTextColor:kColorWhite];
